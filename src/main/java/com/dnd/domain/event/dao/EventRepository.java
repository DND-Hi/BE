...
public interface EventRepository extends JpaRepository<Event, Long> {
    @Query(nativeQuery = true, value =
            "SELECT event_id, title, description, host, longitude, latitude, start_at, finish_at FROM event e "
    ...
    @Modifying
    @Query(value = "INSERT INTO event (title, description, host, location, longitude, latitude, start_at, finish_at) " +
            "VALUES (:title, :description, :host, ST_PointFromText(:point), :longitude, :latitude, :startAt, :finishAt)", nativeQuery = true)
    void saveWithPoint(
